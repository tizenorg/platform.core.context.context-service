...
bool ctx::context_manager_impl::handle_custom_request(ctx::request_info* request)
{
	...
	IF_FAIL_RETURN(request->get_type() == REQ_SUPPORT_CUSTOM ||
					subject == CONTEXT_TRIGGER_SUBJECT_CUSTOM_ADD ||
	...
	int error = ERR_NONE;
	if (request->get_type() == REQ_SUPPORT_CUSTOM) {
		std::string subj = pkg_id + std::string("::") + request->get_subject();

		auto it = provider_handle_map.find(subj);
		if (it == provider_handle_map.end()) {
			request->reply(ERR_NOT_SUPPORTED);
		} else {
			ctx::json result;
			result.set(NULL, CT_CUSTOM_SUBJECT, subj);
			request->reply(error, result);
		}
		return true;
	} else if (subject == CONTEXT_TRIGGER_SUBJECT_CUSTOM_ADD) {
		std::string name;
		ctx::json tmpl;
		desc.get(NULL, CT_CUSTOM_NAME, &name);
		desc.get(NULL, CT_CUSTOM_ATTRIBUTES, &tmpl);

		error = ctx::custom_context_provider::add_item(name, tmpl, pkg_id);
	} else if (subject == CONTEXT_TRIGGER_SUBJECT_CUSTOM_REMOVE) {
		std::string subj;
		desc.get(NULL, CT_CUSTOM_SUBJECT, &subj);
