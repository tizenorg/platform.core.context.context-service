...
namespace ctx {
	...
	public:
		...
		static void publish(std::string dest, int req_id, std::string subject, int error, std::string data);
	...
	private:
		...
		bool __init();
		void __release();
		void __publish(const char *dest, int req_id, const char *subject, int error, const char *data);
