...
bool ContextManager::registerProvider(const char *subject, const char *privilege, IContextProvider *provider)
{
	...
	ProviderHandler *handle = new(std::nothrow) ProviderHandler(it->first.c_str(), privilege, provider);
