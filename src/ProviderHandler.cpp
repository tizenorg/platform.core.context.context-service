...
ProviderHandler::ProviderHandler(const char *subject, const char *privilege, IContextProvider *provider) :
	__subject(subject),
...
IContextProvider* ProviderHandler::__getProvider(RequestInfo *request)
{
...
void ProviderHandler::subscribe(RequestInfo *request)
{
	...
	IContextProvider *provider = __getProvider(request);
...
void ProviderHandler::unsubscribe(RequestInfo *request)
{
	...
	int error = provider->unsubscribe(__subject, reqFound->getDescription());
	request->reply(error);
	delete request;
	delete reqFound;
}
