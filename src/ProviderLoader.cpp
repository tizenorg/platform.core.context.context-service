...
static bool getSharedObjectPaths(const char *dirPath, std::set<std::string> &soNames)
{
	...
	while (true) {
		...
		if (STR_EQ(dirEntry.d_name, ".") || STR_EQ(dirEntry.d_name, ".."))
			continue;
...
ContextProvider* ProviderLoader::__load(const char *soPath, const char *subject)
{
	...
	void *soHandle = dlopen(soPath, RTLD_LAZY);
	IF_FAIL_RETURN_TAG(soHandle, NULL, _E, "Failed to dlopen(%s) : %s", soPath, dlerror());

	create_t create = reinterpret_cast<create_t>(dlsym(soHandle, "create"));
	if (!create) {
		_E("No \"create\" symbol in '%s'", soPath);
		dlclose(soHandle);
		return NULL;
	}
...
bool ProviderLoader::loadAll()
{
	...
	getSharedObjectPaths(PROVIDER_SO_PATH, soNames);
